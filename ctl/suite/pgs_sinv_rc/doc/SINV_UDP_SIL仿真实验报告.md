...
2. BUILD_LEVEL 2：电阻负载下的正弦电流闭环和重复控制对比；该级电流参考由内部 ramp 产生，PLL/PQ 仅作为后台监测，不参与电流参考生成；
...
| BL2 | RLOAD | ≥3.0 s | 电流跟踪约 2 Arms；内部 ramp 频率正确；PLL 频率仅作后台监测；可比较 RC 开/关 THD |
...
严格按 1→5 顺序推进。BL1 先排除硬件接口问题；BL2 确认内部 ramp 正弦电流环和可选 RC 效果；BL3 才确认 PLL 参与的并网方向和双向能量流；BL4/5 才投入慢外环。
