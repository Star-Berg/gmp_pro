...
class InverterReadyInterlockTests(unittest.TestCase):
    def test_sdpe_exposes_disabled_active_low_ready_output(self):
        header = SDPE_HEADER.read_text(encoding="utf-8")
        requirement = json.loads(SDPE_REQUIREMENT.read_text(encoding="utf-8"))

        self.assertIn("#define SINV_INVERTER_READY_OUTPUT_ENABLE (0)", header)
        self.assertIn("#define SINV_INVERTER_READY_GPIO IRIS_GPIO2", header)
        self.assertIn("#define SINV_INVERTER_READY_ACTIVE_LEVEL (0U)", header)
        self.assertIn("#define SINV_INVERTER_READY_STABLE_MS (100U)", header)
        self.assertIn("#define SINV_INVERTER_READY_VBUS_TOLERANCE_V (1.0f)", header)

        options = {item["macro"]: item for item in requirement["option_macros"]}
        self.assertEqual(options["SINV_INVERTER_READY_OUTPUT_ENABLE"]["value"], "(0)")
        self.assertEqual(options["SINV_INVERTER_READY_GPIO"]["value"], "IRIS_GPIO2")
        self.assertEqual(options["SINV_INVERTER_READY_ACTIVE_LEVEL"]["value"], "(0U)")
    ...
    def test_platform_output_defaults_to_inactive_high(self):
        source = XPLT_C.read_text(encoding="utf-8", errors="ignore")
        self.assertIn("GPIO_writePin(SINV_INVERTER_READY_GPIO, 1U);", source)
