...
class IrisOperatorPanelTests(unittest.TestCase):
    ...
    def test_oled_shows_frequency_voltage_reference_bus_and_switch_state(self):
        ...
        self.assertIn("marker_count = (profile >= 2U) ? (profile - 1U) : 0U", USER_MAIN_C)
        self.assertIn("marker_start = 16U - marker_count", USER_MAIN_C)
        self.assertIn("for (i = marker_start; i < 16U; ++i)", USER_MAIN_C)
