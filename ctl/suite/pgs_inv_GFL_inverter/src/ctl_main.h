...
#if BUILD_LEVEL == 6
        // Stand-alone voltage source: the DC-offset servo is integrated into
        // the voltage-loop current-reference path so the correction remains
        // inside the existing current QPR loop and its current-vector limit.
        ctl_step_offgrid_voltage_ctrl_with_dc_servo(
            &offgrid_voltage_ctrl,
            &inv_ctrl,
            inv_ctrl.flag_enable_system);
